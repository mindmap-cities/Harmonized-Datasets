...
change_class <- function(tbl, dd, name_stdy){
	...
	# test_same_class   
	odd_classes <- anti_join(class_tbl,class_dd) %>%
	...
	try({names(class_dd) = c(class_dd[[1]]$valueType %>% unique)})
	try({names(class_dd) = c(class_dd[[1]]$valueType %>% unique,
	...
	try({names(class_dd) = c(class_dd[[1]]$valueType %>% unique,
													 class_dd[[2]]$valueType %>% unique,
													 class_dd[[3]]$valueType %>% unique,
													 class_dd[[4]]$valueType %>% unique)})
	try({names(class_dd) = c(class_dd[[1]]$valueType %>% unique,
													 class_dd[[2]]$valueType %>% unique,
													 class_dd[[3]]$valueType %>% unique,
													 class_dd[[4]]$valueType %>% unique,
													 class_dd[[5]]$valueType %>% unique)})
	# get classes :
	message("Before: \n")
	print(tbl %>% 
					dplyr::summarise_all(typeof) %>% 
	...
	try({tbl <- tbl %>% modify_at(class_dd$text$name,    as.character) })
	try({tbl <- tbl %>% modify_at(class_dd$integer$name, as.integer  ) })
	try({tbl <- tbl %>% modify_at(class_dd$decimal$name, as.numeric  ) })
...
order_dd <- function(dd){
	...
	dd$Variables <-  
		...
		dd$Variables %>% 
		filter(str_detect(name, "^baseline_yr$"))   %>%
		bind_rows(dd$Variables %>% filter(str_detect(name, "^followup1_yr$"))) %>% 
		bind_rows(dd$Variables %>% filter(str_detect(name, "^followup2_yr$"))) %>% 
		bind_rows(dd$Variables %>% filter(str_detect(name, "^followup3_yr$"))) %>% 
		bind_rows(dd$Variables %>% filter(str_detect(name, "^followup4_yr$"))) %>% 
		bind_rows(dd$Variables %>% filter(str_detect(name, "^followup5_yr$"))) %>% 
		bind_rows(dd$Variables %>% filter(str_detect(name, "^followup6_yr$"))) %>% 
		
		bind_rows(dd$Variables %>% filter(str_detect(name, "^t1$"))) %>% 
		bind_rows(dd$Variables %>% filter(str_detect(name, "^t2$"))) %>% 
		bind_rows(dd$Variables %>% filter(str_detect(name, "^t3$"))) %>% 
		bind_rows(dd$Variables %>% filter(str_detect(name, "^t4$"))) %>% 
		bind_rows(dd$Variables %>% filter(str_detect(name, "^t5$"))) %>% 
		bind_rows(dd$Variables %>% filter(str_detect(name, "^t6$"))) %>%
		
		bind_rows(dd$Variables %>% filter(str_detect(name, "_0$"))) %>% 
	...
	dd$Categories <-  
		dd$Categories %>% filter(str_detect(variable, "_0$")) %>%
